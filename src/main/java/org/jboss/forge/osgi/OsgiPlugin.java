...
public class OsgiPlugin implements Plugin {
	...
	private void createActivator(MavenPluginBuilder bundlePlugin, boolean useDM) {
		...
		if (useDM) {
			...
			velocityEngine.mergeTemplate("ActivatorDependencyManagerTemplate.vtl", "UTF-8", context, writer);
			installFelixDmDependency();
		} else {
			velocityEngine.mergeTemplate("ActivatorTemplate.vtl", "UTF-8", context, writer);
		}
	...
	private Map<String, String> createDmComponent(boolean useLogService) {
		...
		velocityEngine.mergeTemplate("DmComponentTemplate.vtl", "UTF-8", context, writer);
